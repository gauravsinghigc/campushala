 <div class="card card-body p-2">
   ...
   <div class="data-display-2 height-limit">
     <ul>
       <?php
        $AllPayments = FETCH_DB_TABLE("SELECT * FROM registration_payments, registrations where registrations.RegMainCustomerId='$ViewCustomerId' and registrations.RegistrationId=registration_payments.RegMainId ORDER BY RegPaymentId DESC", true);
        if ($AllPayments != null) {
          $SerialNo = 0;
           ...
           <li class='data-list'>
             <p class="flex-s-b">
               <span class='w-pr-15 text-left'>
                 <span class='btn btn-sm text-info m-t-5'><?php echo DATE_FORMATE("d M, Y", $Payment->RegPaymentDate); ?></span>
               </span>
               <span class='w-pr-18'>
               ...
               <span class='w-pr-5 p-1 text-right'>
                 <?php
                  ...
                  if ($PayModes == "ONLINE_TRANSFER") {
                    $UpdateSection = $Dir . "/include/sections/Update-Online-Payment-Details.php";
                  } elseif ($PayModes == "CHEQUE_DD") {
                    $UpdateSection = $Dir . "/include/sections/Update-Cheque-Payment-Details.php";
                  } elseif ($PayModes == "WALLET_UPI") {
                    $UpdateSection = $Dir . "/include/sections/Update-Wallet-Payments.php";
                  } elseif ($PayModes == "CASH") {
                    $UpdateSection = $Dir . "/include/sections/Update-Cash-Payments.php";
                  } else {
                    $UpdateSection = "";
                  }
                  ?>
