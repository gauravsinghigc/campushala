...
if (isset($_POST['CreateLeads'])) {
  $Tablerows = array(
    ...
    "LeadPersonManagedBy", "LeadPriorityLevel", "LeadPersonLastUpdatedAt"
  );
  ...
  if ($_POST['bulkselect'] != "null") {
    ...
    $FETCH = FETCH_DB_TABLE("SELECT * FROM lead_uploads where LeadStatus='UPLOADED' ORDER BY leadsUploadId $orderby limit 0, $totalleadcounts", true);
    if ($FETCH != null) {
    ...
    foreach ($_POST['Leads'] as $values) {
      $FETCH = FETCH_DB_TABLE("SELECT * FROM lead_uploads where leadsUploadId='$values'", true);
      if ($FETCH != null) {
        foreach ($FETCH as $leads) {
          $data = array(
            "LeadPersonFullname" => $leads->LeadsName,
