...
if (isset($_GET['view'])) {
  ...
  if (LOGIN_UserType == "Admin" || LOGIN_UserType == "Digital") {
    $TotalItems = TOTAL("SELECT LeadsId FROM leads WHERE $Managed LeadPersonSource like '%$LeadPersonSource%' and LeadPriorityLevel like '%$LeadPriorityLevel%' and LeadPersonPhoneNumber like '%$LeadPersonPhoneNumber%' and LeadPersonFullname like '%$LeadPersonFullname%' and LeadPersonSubStatus like '%$LeadPersonSubStatus%' and LeadPersonStatus LIKE '%$LeadPersonStatus%'");
  } else {
    $LOGIN_UserId = LOGIN_UserId;
