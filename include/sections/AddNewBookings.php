<?PHP
$GetLatestEmpID = FETCH_DB_TABLE("SELECT * FROM bookings ORDER BY bookingId DESC", true);
    ...
    <div class='container'>
      ...
      <form class="row" enctype="multipart/form-data" action="<?php echo CONTROLLER; ?>/BookingController.php" method="POST">
        ...
        <div class='col-md-6'>
          <div class="flex-s-b">
            <div class="form-group w-100">
              ...
              <input type="text" name="BookingCustomerName" required="" class="form-control form-control-sm">
            </div>
            <div class="form-group w-100 m-l-5">
              <label>Customer Phone <span id="phonemsg"></span></label>
              <input type="text" oninput="CheckPhoneNumbers()" id="PhoneNumber" name="BookingCustomerPhone" required="" class="form-control form-control-sm">
            </div>
          </div>
        </div>
        <div class='col-md-6'>
        ...
        <div class="col-md-3 form-group">
          ...
          <input type="text" name="BookingAckCode" value="REG00<?php echo $SortedArray[0] + 1; ?>" class="form-control form-control-sm" required="">
        </div>
        ...
        <div class="col-md-4 form-group">
          ...
          <select name="BookingTeamHeadId" class="form-control form-control-sm" required="">
            ...
            <option value="1">Assign Admin</option>
            <?php
            $AllCustomers = FETCH_DB_TABLE("SELECT * FROM users, user_employment_details where users.UserId=user_employment_details.UserMainUserId ORDER BY UserFullName Desc", true);
            if ($AllCustomers != null) {
              $Sno = 0;
              foreach ($AllCustomers as $Customers) {
                $Sno++;
...
<script>
  function CheckPhoneNumbers() {
    ...
    let ExistingPhoneNumbers = [<?php
                                $AllData = FETCH_DB_TABLE("SELECT * FROM bookings", true);
                                if ($AllData != null) {
                                  foreach ($AllData as $Data) {
