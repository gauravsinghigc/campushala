...
if (isset($_POST['CreateCirculars'])) {
  ...
  if ($Response == true) {
    ...
    if ($_FILES['CircularDocumentFile']['name'] != null) {
      $CircularDocumentFile = UPLOAD_FILES("../storage/circulars", null, "Circular", $_POST['CircularName'], "CircularDocumentFile");
  ...
  if (isset($_POST['MailSent'])) {
    if ($_POST['MailSent'] == "Send") {
      ...
      $AllUsers = FETCH_DB_TABLE("SELECT * FROM users where UserStatus='1'", true);
      if ($AllUsers != null) {
